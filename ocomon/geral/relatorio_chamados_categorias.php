<?php 
  ...
  */session_start();
	...
	if (!isset($_POST['ok']) && !isset($_GET['action'])  ) { //&& $_POST['ok'] != 'Pesquisar')
		...
		print "				<tr>";
		print "					<td bgcolor=".TD_COLOR.">".TRANS('OCO_FIELD_LOCAL').":</td>";
		print "					<td class='line'><Select name='local' class='select'>";
		print "							<OPTION value=-1 selected>".TRANS('OPT_ALL')."</OPTION>";
		...
		print "							<option value=-1 selected>".TRANS('SEL_PRIORITY_NORMAL')."</option>";
			print "							<option value=1>Relatório Teste</option>";
		print "						</select>";
		print "					</td>";
		print "				</tr>";
		print "		</TABLE><br>";
	...
	if (isset($_POST['ok'])) {//if $ok!=Pesquisar
		...
		//LRG
		if (isset($_POST['local']) && ($_POST['local'] != -1))
		{
			$query .= " and o.local = '".$_POST['local']."' ";
			$qry_criterio = "SELECT local FROM localizacao WHERE loc_id = ".$_POST['local']." ";
			$exec_criterio = mysql_query($qry_criterio);
			$row_criterio = mysql_fetch_array($exec_criterio);
			$criterio .= " Local = ".$row_criterio['local'].",";
		...
		if ((empty($_POST['d_ini'])) || (empty($_POST['d_fim'])))
		{
			...
			if ($linhas==0) {
				...
				switch($_POST['saida'])
				{
					...
					$cabs[]="Quantidade";
					$cabs[]="Área";
					$cabs[]="Tipo de Problema";
					$cabs[]="Origem do Problema";
					$cabs[]="Operacional";
					...
					$msg1="";
					$msg2=date('d/m/Y H:m');
					$msg3= "Relatório: Quantidade X Tipo de Problemas";
					
					$sum = "quantidade";

					gera_relatorio(1, $query, $campos, $cabs, $logo, $msg1, $msg2, $msg3, $sum);
