  ...
  */
	...
	if (isset($_POST['submit'])) {
		...
		if ($regs > 0) {
			...
			$random = random();
			$query= "INSERT INTO utmp_usuarios (utmp_login, utmp_nome, utmp_email, utmp_passwd, utmp_rand) values ".
					"('".noHtml($_POST['login'])."','".noHtml($_POST['nome'])."','".$_POST['email']."', md5('".$_POST['senha']."'),'".$random."')";
					
			$exec = mysql_query($query) or die ($query .'<br><br> ERRO NA TENTATIVA DE CRIAR USUÁRIO TEMPORÁRIO. SUA SOLICITAÇÃO NÃO FOI PROCESSADA!');
