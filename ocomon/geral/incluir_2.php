...
// 							if ($_POST['radio_prob'] == $radio_prob) print " checked";
		...
		if ($rowconf['conf_scr_local'] || !isIn($_SESSION['s_area'],$rowconf['conf_custom_areas'])) {
			...
			if ($rowconf['conf_scr_searchbylocal'] || !isIn($_SESSION['s_area'],$rowconf['conf_custom_areas'])) {
				print "</SELECT><a onClick=\"checa_por_local()\"><img title='".TRANS('CONS_EQUIP_LOCAL')."' width='15' height='15' src='".$imgsPath."consulta.png' border='0'></a>";
			}
	                print "</TD>";
		} else $local = -1;
	...
	function valida(){
		...
		if (!LOAD) {
			...
			if (ok) {
				if (tag != null){
					if (operador == 1){
						var ok = validaForm('idEtiqueta','ALFAFULL','Etiqueta',0);
					} else {
						var ok = validaForm('idEtiqueta','ALFAFULL','Etiqueta',1);
					}
				} else ok = true;
			}
			if (ok){
