...
*/session_start();
	...
	if (!isset($_POST['ok'])) { //&& $_POST['ok'] != 'Pesquisar')
		...
		print "			<tr>";
		print "	            <td>";
		print "					<input type='submit'  class='button_pesquisar' value='".TRANS('BT_SEARCH')."' name='ok' >";//onClick=\"submitForm();\"
		print "	            </TD>";
		print "	            <td>";
		print "					<INPUT type='reset'  class='button_limpar' value='".TRANS('BT_CLEAR')."' name='cancelar'>";
		print "				</TD>";
		print "			</tr>";
		print "	    </TABLE>";
		print "</form>";
		...
		if ((!isset($_POST['d_ini'])) || ((!isset($_POST['d_fim'])))) {
					...
					switch($_POST['saida'])
					{
						...
						if (!empty($_POST['local']) and ($_POST['local'] !=-1)){
							...
							print "<p class='titulo'>".TRANS('TLT_REP_SLAS_INDICES')."".$criterio."</p>";
                            			print "<table align='center' class='centro' cellspacing='0' border='1' style='border-color: #fff;'>";

                           	 		print "<tr class='header' style='border-color: #fff;'><td style='padding:5px;' ><B>".TRANS('OCO_FIELD_NUMBER')."</td>
							<td style='padding:5px;'><b><a title='tempo de resposta'>".TRANS('COL_TIT_TEMP_VAL_RESP')."</a></td>
							<td style='padding:5px;'><b><a title='tempo de solução'>".TRANS('COL_TIT_TEMP_VAL_SOL')."</a></td></B>
							<td style='padding:5px;'><b><a title='tempo definido para resposta para cada setor'>".TRANS('COL_TIT_SLA_RESP')."</a></td></B>
							<td style='padding:5px;'><b><a title='tempo definido para solução para cada problema'>".TRANS('COL_TIT_SLA_SOL')."</a></td></B>
							<td style='padding:5px;'><b><a title='indicador de resposta'>".TRANS('COL_TIT_REPLY')."</a></td></B>
							<td style='padding:5px;'><b><a title='indicador de solução'>".TRANS('COL_TIT_SOLUTION')."</a></td></B>
							<td style='padding:5px;'><b><a title='indicador de solução a partir da primeira resposta'>".TRANS('COL_SOL_RESP')."</a></td></B>
							<td style='padding:5px;'><b><a title='tempo em que o chamado esteve pendente no usuário'>".TRANS('COL_USER_DEPEN')."</a></td></B>
							<td style='padding:5px;'><b><a title='tempo em que o chamado esteve pendente por algum serviço de terceiros'>".TRANS('COL_DEPEN_THIRD')."</a></td></B>
							<td style='padding:5px;'><b><a title='tempo em equipamento de backup ou alterado após encerramento'>".TRANS('COL_IT_ARRE_DEPEN')."</a></td></B>
							<td style='padding:5px;'><b><a title='Tempo de solução menos o tempo em pendência do usuário'>".TRANS('COL_RECALC_SOLUTION')."</a></td></B>
							<td style='padding:5px;'><b><a title='indicador atualizado descontando a pendência do usuário'>".TRANS('COL_POINTER_UPDATE')."</a></td></B>
							</tr>";
						...
						while ($row = mysql_fetch_array($resultado)) {
							...
							print "<tr style='border-bottom: solid 1px; border-color: #ddd;' id='linha".$cont."'  onMouseDown=\"marca('linha".$cont."', '".$_SESSION['s_colorMarca']."');\">";

							print "<td ><a style='cursor:pointer;' onClick= \"javascript: popup_alerta('mostra_consulta.php?popup=true&numero=".$row['numero']."')\"><font color='blue'>".$row['numero']."</font></a></td>
								<td ><font color='".$corR."'>".$dtR->tValido."</font></td>
								<td ><font color='".$corR."'>".$dtS->tValido."</font></td>
								<td >".$row['resposta_desc']."</font></td>
								<td >".$row['sla']."</font></td>
								<td align='center'><a style='cursor:pointer;' onClick=\"javascript:popup('mostra_hist_status.php?popup=true&numero=".$row['numero']."')\"><img height='14' width='14' src='../../includes/imgs/".$imgSlaR."'></a></td>
								<td align='center'><a style='cursor:pointer;' onClick=\"javascript:popup('mostra_hist_status.php?popup=true&numero=".$row['numero']."')\"><img height='14' width='14' src='../../includes/imgs/".$imgSlaS."'></a></td>
								<td align='center'><a style='cursor:pointer;' onClick=\"javascript:popup('mostra_hist_status.php?popup=true&numero=".$row['numero']."')\"><img height='14' width='14' src='../../includes/imgs/".$imgSlaM."'></a></td>";
						...
						print "<tr><td colspan=5 style='padding:10px;'><b>".TRANS('COL_AVERAGE')."</td><td style='padding:10px;'><b>".$media_resposta_valida."</td><td style='padding:10px;'><B>".$media_solucao_valida."</td ><td colspan=6></td></tr>";
			...
			## QUADROS DE ESTATÍSTICAS
                     print "<table align='center' border='1' cellspacing='0' style='border-color:#ddd;'><tr><td>";
			print "<table align='center' cellspacing='0' border='0'>";
