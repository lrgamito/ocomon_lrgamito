	...
	if (AUTH_TYPE == "LDAP") {
		...
		if ((senha_ldap($_POST['login'],$_POST['password'],'usuarios')=="ok") && ($conec->userLDAP($_POST['login'],$_POST['password']) || $conecSec->userLDAP($_POST['login'],$_POST['password'])))
		{
		        if(isset($_POST['login'])){
					$s_usuario= mysql_real_escape_string($_POST['login']);
				}
				
				if(isset($_POST['password'])){
					$s_senha= mysql_real_escape_string($_POST['password']);
				}

			$queryOK = "SELECT u.*, n.*,s.* FROM usuarios u left join sistemas as s on u.AREA = s.sis_id ".
							"left join nivel as n on n.nivel_cod =u.nivel WHERE u.login = '".$s_usuario."'";

			$resultadoOK = mysql_query($queryOK) or die(utf8_decode('IMPOSSÍVEL ACESSAR A BASE DE DADOS DE USUÁRIOS: LOGIN.PHP'));
			...
			$execUa = mysql_query($qryUa) or die(utf8_decode('IMPOSSÍVEL ACESSAR A BASE DE USUÁRIOS 02: LOGIN.PHP'));
			$uAreas = "".$s_area.",";
			...
			/*CHECA QUAIS OS M�DULOS PODEM SER ACESSADOS PELAS �REAS QUE O USUÁRIO PERTENCE*/
			$qry = "SELECT * FROM permissoes where perm_area in (".$uAreas.")";
			$exec = mysql_query($qry) or die(utf8_decode('IMPOSSÍVEL ACESSAR A BASE DE PERMISSÕES: LOGIN.PHP'));
			...
			$execFormatBar = mysql_query($sqlFormatBar) or die (utf8_decode('NÃO FOI POSSÍVEL ACESSAR A TABELA DE CONFIGURAÇÕES DO SISTEMA!'));
			...
			$execStyles = mysql_query($sqlStyles) or die(utf8_decode('ERRO NA TENTATIVA DE RECUPERAR AS INFORMAÇÕES DO TEMA!<BR>'.$sqlStyles));
			...
			print "<script>redirect('../../index.php?usu=".$s_usuario."&inv=1');</script>";
			$conec->desconLDAP();
			$conecSec->desconLDAP();
			exit;
		}
		...
		if (senha_system($_POST['login'],$_POST['password'],'usuarios')=="ok")
		{
			...
			/*CHECA QUAIS OS M�DULOS PODEM SER ACESSADOS PELAS ÁREAS QUE O USUÁRIO PERTENCE*/
			$qry = "SELECT * FROM permissoes where perm_area in (".$uAreas.")";
			$exec = mysql_query($qry) or die(utf8_decode('IMPOSSÍVEL ACESSAR A BASE DE PERMISSÕES: LOGIN.PHP'));
			...
			$rowSty = mysql_fetch_array($execStyles);
			$regs = mysql_num_rows($execStyles);
			if ($regs==0){ //SE NÃO ENCONTROU TEMA ESPECÍFICO PARA O USU�RIO
				unset($rowSty);
				$sqlStyles = "SELECT * FROM styles";
				$execStyles = mysql_query($sqlStyles);
				$rowSty = mysql_fetch_array($execStyles);
			...
			###############################################
			## TRECHO PARA VERIFICA��O DAS GARANTIAS ##
