  ...
  */session_start();
	...
	$linhasInst = mysql_num_rows($resultadoInst);
		...
		$query= "select count(l.local)as qtd, count(*)/".$total."*100 as porcento,
			...
			and (c.comp_local = l.loc_id) and (t.tipo_cod in (5)) ".$clausula2.") group by local,tipo order by qtd desc ,
			local asc";
		...
		print "<TABLE border='0' cellpadding='5' cellspacing='0' align='left' width='80%' bgcolor='$cor3'>";
			...
			print "<tr><td class='line'></TD></tr>";
			print "<tr><td width='80%' align='center'><b>".TRANS('TTL_TOTAL_MON_CAD_SECTOR')." <p>".TRANS('OCO_FIELD_UNIT').": ".$msgInst.".</p></b></td></tr>";

			print "<td class='line'>";
			print "<fieldset><legend>".TRANS('TTL_MON_X_SECTOR')."</legend>";
