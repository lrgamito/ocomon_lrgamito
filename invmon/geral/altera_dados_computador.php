<?php 
  ...
  */session_start();
	...
	print "<TABLE border='0'  align='left' width='1100'>";
		...
		print "<tr>";
			...
			print "<option value=0>".TRANS('SEL_TYPE_EQUIP')."</option>";
			...
			print "<option value=0>".TRANS('SEL_MANUFACTURE')."</option>";
			while ($rowqry = mysql_fetch_array($execqry)){
		...
		print "<tr>";
			...
			print "<option value=0>".TRANS('SEL_MODEL')."</option>";
			while ($rowqry = mysql_fetch_array($execqry)){
			...
			print "<option value=0>".TRANS('OCO_SEL_LOCAL')."</option>";
			while ($rowqry = mysql_fetch_array($execqry)){
		...
		print "<tr>";
			...
			print "<option value=0>".TRANS('SEL_SITUAC')."</option>";
			while ($rowqry = mysql_fetch_array($execqry)){
		...
		print "<tr>";
			...
			//CD-ROM\DVD
			//alt by LRG
			print "<TD width='20%' align='left' bgcolor='".TD_COLOR."'>".TRANS('FIELD_CDROM')."/".TRANS('MNL_DVD').":</TD>";
...
//			print "</TD>";
			...
			//Leitor de Cartão
			print "<TD width='20%' align='left' bgcolor='".TD_COLOR."'>Leitor de Cartão:</TD>";

			$qry = "select * from modelos_itens where mdit_tipo = 13 order by mdit_fabricante, mdit_desc";
			$execqry = mysql_query($qry);

			print "<TD width='30%' align='left' bgcolor='".BODY_COLOR."'>";
			print "<SELECT class='select' name='leitor' size=1 id='idLeitor'>";
			print "<option value=0>".TRANS('SEL_MODEL')."</option>";
			while ($rowqry = mysql_fetch_array($execqry)){
				print "<option value='".$rowqry['mdit_cod']."'";
				if ($rowqry['mdit_cod'] == $row['cod_leitor']){
					print " selected";
				}
				print ">".$rowqry['mdit_fabricante']." ".$rowqry['mdit_desc']." ".$rowqry['mdit_desc_capacidade']." ".$rowqry['mdit_sufixo']."</option>";
			}
...
//			print "</TD>";
		...
		print "<tr>";			
			//OS
			print "<TD width='20%' align='left' bgcolor='".TD_COLOR."'>Sistema Operacional:</TD>";

			$qry = "select * from modelos_itens where mdit_tipo = 14 order by mdit_fabricante, mdit_desc";
			$execqry = mysql_query($qry);

			print "<TD width='30%' align='left' bgcolor='".BODY_COLOR."'>";
			print "<SELECT class='select' name='os' size=1 id='idOs'>";
			print "<option value=0>".TRANS('SEL_MODEL')."</option>";
			while ($rowqry = mysql_fetch_array($execqry)){
				print "<option value='".$rowqry['mdit_cod']."'";
				if ($rowqry['mdit_cod'] == $row['cod_os']){
					print " selected";
				}
				print ">".$rowqry['mdit_fabricante']." ".$rowqry['mdit_desc']." ".$rowqry['mdit_desc_capacidade']." ".$rowqry['mdit_sufixo']."</option>";
			}
			...
			//Serial OS
			print "<TD width='20%' align='left' bgcolor='".TD_COLOR."'>Serial OS:</TD>";
			print "<TD width='30%' align='left' bgcolor='".BODY_COLOR."'><INPUT type='text' class='text' name='sn_os' value='".$row['sn_os']."' ></TD>";
			print "</TD>";
		...
		print "<tr>";
			...
			print "<option value=0>Selecione o tipo</option>";
			while ($rowqry = mysql_fetch_array($execqry)){
			...
			print "<option value=0>Selecione a tela</option>";
			while ($rowqry = mysql_fetch_array($execqry)){
		...
		print "<tr>";
			...
			print "<option value=0>".TRANS('SEL_RESOLUT_SCANNER')."</option>";
			while ($rowqry = mysql_fetch_array($execqry)){
		...
		print "<tr>";
			...
			if ($_SESSION['s_nivel']!=1) {
				...
				print " class='select' ";
			print ">";
			print "<option value=0>".TRANS('OCO_SEL_UNIT')."</option>";
			while ($rowqry = mysql_fetch_array($execqry)){
			...
			print "<option value=0>".TRANS('SEL_CENTER_COST')."</option>";
			while ($rowqry = mysql_fetch_array($execqry)){
		...
		print "<tr>";
			...
			print "<option value=0>".TRANS('SEL_VENDOR')."</option>";
			while ($rowqry = mysql_fetch_array($execqry)){
		...
		print "<tr>";
			...
			print "<option value=0>".TRANS('SEL_TYPE_GUARAT')."</option>";
			while ($rowqry = mysql_fetch_array($execqry)){
			...
			print "<option value=0>".TRANS('SEL_TIME_GUARANT')."</option>";
			while ($rowqry = mysql_fetch_array($execqry)){
	...
	if (isset($_POST['submit']))
	{
		...
		if (!$erro)
		{
			...
			$query = "UPDATE equipamentos SET comp_sn='".noHtml($_POST['serial'])."', comp_marca='".$_POST['modelo']."', comp_mb=".$_POST['mb'].",
						...
						comp_dvd=0, comp_grav=0, comp_nome='".noHtml($_POST['nome'])."', comp_local=".$_POST['local'].",
						...
						comp_leitor=".$_POST['leitor'].", comp_os=".$_POST['os'].", comp_sn_os='".$_POST['sn_os']."',
			...
			$resultado4 = mysql_query($query) or die('ERRO NA TENTATIVA DE GRAVAR AS ALTERAÇÕES DO REGISTRO!<BR>'.$query);
					...
					//GRAVA LOG
						...
						$mensagem_log  	= "Alteracao de Equipamento - ETIQUETA: ".$_POST['comp_inv']." - INSTITUICAO: ".$_POST['cod_inst'];
						$mensagem_log 	= $mensagem_log." | ".$_POST['serial']." | ".$_POST['modelo']." | ".$_POST['local']." | ".$_POST['situacao'];
...
//##########		LRG
				if (($_POST['leitor'] != $row['tipo_leitor']) and ( $row['tipo_leitor']!=-1 and $row['tipo_leitor']!=0 and $row['tipo_leitor']!='') ) {
					$qry = "INSERT INTO hw_alter (hwa_inst, hwa_inv, hwa_item, hwa_user, hwa_data)
							VALUES (".$row['cod_inst'].", ".$row['etiqueta'].", '".$row['tipo_leitor']."', ".$_SESSION['s_uid'].", '".date('Y-m-d H:i:s')."')";
					$execQry = mysql_query($qry) or die ('ERRO NA TENTATIVA DE REGISTRAR A ALTERAÇÃO DE HW!<BR>'.$qry);
				}

				if (($_POST['os'] != $row['tipo_os']) and ( $row['tipo_os']!=-1 and $row['tipo_os']!=0 and $row['tipo_os']!='') ) {
					$qry = "INSERT INTO hw_alter (hwa_inst, hwa_inv, hwa_item, hwa_user, hwa_data)
							VALUES (".$row['cod_inst'].", ".$row['etiqueta'].", '".$row['tipo_os']."', ".$_SESSION['s_uid'].", '".date('Y-m-d H:i:s')."')";
					$execQry = mysql_query($qry) or die ('ERRO NA TENTATIVA DE REGISTRAR A ALTERAÇÃO DE HW!<BR>'.$qry);
				}

				if (($_POST['sn_os'] != $row['tipo_sn_os']) and ( $row['tipo_sn_os']!=-1 and $row['tipo_sn_os']!=0 and $row['tipo_sn_os']!='') ) {
					$qry = "INSERT INTO hw_alter (hwa_inst, hwa_inv, hwa_item, hwa_user, hwa_data)
							VALUES (".$row['cod_inst'].", ".$row['etiqueta'].", '".$row['tipo_sn_os']."', ".$_SESSION['s_uid'].", '".date('Y-m-d H:i:s')."')";
					$execQry = mysql_query($qry) or die ('ERRO NA TENTATIVA DE REGISTRAR A ALTERAÇÃO DE HW!<BR>'.$qry);
				}
...
//##########
				...
				if ($gravaImg) {
					...
					if(chop($fileinput)!=""){
						...
						$SQL = "Insert Into imagens (img_nome, img_inst, img_inv, img_tipo, img_bin, img_largura, img_altura, img_size) values ".
								//"('".$_FILES['img']['name']."', '".$_POST['instituicao']."', '".$_POST['comp_inv']."','".$_FILES['img']['type']."', '".$image."', ".$tamanho[0].", ".$tamanho[1].")";
								"('".$_FILES['img']['name']."', '".$row['cod_inst']."', '".$_POST['comp_inv']."','".$_FILES['img']['type']."', '".$image."', ".$tamanho[0].", ".$tamanho[1].", ".$tamanho2.")";
					...
					$exec = mysql_query($SQL) or die ("NÃO FOI POSSÍVEL GRAVAR O ARQUIVO NO BANCO DE DADOS!<br>".  mysql_error()." ");
				...
				if (($ccusto1!= $ccusto2)and ($ccusto2!="null")) {$texto.= " centro de custo alterado de '".$row['ccusto']."' para '".$ccusto2."' |";}
					...
					//GRAVA LOG
						$mensagem_log  	= "Alteracao de Hardware de Equipamento - ";
						$usu_logado 	= $_SESSION['s_usuario'];
						$etiqueta		= $_POST['comp_inv'];
						setLog($mensagem_log.$texto,$etiqueta,$usu_logado);
