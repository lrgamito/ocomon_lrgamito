			...
			else
			{
				if (($row['situac_destaque']=='1')) {
					...
					$corDestaque = $_SESSION['s_colorLinImpar'];
                        ...
                        //Verifica se a data do fechamento do chamado de preventiva é maior
                        // que a data minima para a preventiva
                        $data_controle = date_diff2($row['prev_max'], date('Y-m-d' ));
                            
                        if ($data_controle > $row['prev_min']){
                            $alerta2 = "style='color:red;wheigth:bold;'";
                        } else {
                            $alerta2 = "";
                        }
