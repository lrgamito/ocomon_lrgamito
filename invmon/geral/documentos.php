  ...
  */session_start();
	...
	if ((!isset($_GET['action'])) && empty($_POST['submit'])) {
		...
		print "<tr><TD align='left'>".
				"<input type='button' class='button_novo' id='idBtIncluir' value='".TRANS('BT_NEW_RECORD','Cadastrar novo registro',0)."' onClick=\"redirect('".$_SERVER['PHP_SELF']."?action=incluir&cellStyle=true');\">".
			"</TD></tr>";
		$PAGE->execSQL();
	...
	if ((isset($_GET['action'])  && ($_GET['action'] == "incluir") ) && empty($_POST['submit'])) {
		...
		print "</TD>";
		print "<TD align='left' width='80%' bgcolor='".BODY_COLOR."'><INPUT type='reset'  class='button_ca' value='".TRANS('BT_CANCEL')."' name='cancelar' onClick=\"javascript:fecha();\"></TD>";
