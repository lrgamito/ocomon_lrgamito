<?php 
  ...
  */session_start();
	...
	$linhasInst = mysql_num_rows($resultadoInst);
		...
		$queryFull = "SELECT count(*) from equipamentos where comp_tipo_equip in (1,2,12,13) ".$clausula."";
		$resultadoFull = mysql_query($queryFull);
		$totalFull = mysql_result($resultadoFull,0);

		//TOTAL DE EQUIPAMENTOS COM MEMÓRIA
		$queryB = "SELECT count(*) from equipamentos where comp_tipo_equip in (1,2,12,13) and ".
		...
		$query= "select count(*) as qtd, concat(count(*)/".$total."*100,'%') as porcento ,
				t.tipo_nome as equipamento, t.tipo_cod as tipo, m.mdit_cod as tipo_memo,
				 m.mdit_desc_capacidade as memoria 
				 from tipo_equip as t,
				modelos_itens as m, equipamentos as c 
				where c.comp_memo = m.mdit_cod and
