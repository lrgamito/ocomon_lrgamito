<?php 
  ...
  */session_start();
	...
	$auth->testa_user($_SESSION['s_usuario'],$_SESSION['s_nivel'],$_SESSION['s_nivel_desc'],1);
        ...
        print "<TD align='left'>".
        		"<input type='button' class='button_novo' id='idBtIncluir' value='".TRANS('ACT_NEW')."' onClick=\"redirect('".$_SERVER['PHP_SELF']."?action=new&cellStyle=true');\">".
        	"</TD><br><BR>";
	...
	if ((isset($_GET['action']) && $_GET['action']=="alter") && empty($_POST['submit'])) {
		...
		print "<tr><td><input type='submit'  class='button_ok' name='submit' value='".TRANS('BT_EDIT')."'></td>";
		print "<input type='hidden' value='".$_GET['cod']."' name='cod'>";
		print "<td><input type='reset' name='reset' class='button_ca'  value='".TRANS('BT_CANCEL')."' onclick=\"redirect('".$_SERVER['PHP_SELF']."')\"></td></tr>";
	...
	if (isset($_GET['action']) && $_GET['action']=="new"){
		...
		print "<tr><td><input type='submit'  class='button_ok' name='submit' value='".TRANS('BT_CAD')."'></td>";
		print "<td><input type='reset' name='reset' class='button_ca'  value='".TRANS('BT_CANCEL')."' onclick=\"redirect('".$_SERVER['PHP_SELF']."')\"></td></tr>";
