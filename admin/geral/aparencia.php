...
<script language="javascript"> var cp = new ColorPicker(); // DIV style</script>
	...
	if ((empty($_GET['action'])) and empty($_POST['submit'])){
		...
		print "<br><TD align='left'>".
				"<input type='button' class='button_novo' id='idBtIncluir' value='".TRANS('BT_EDIT_CONFIG','Editar Configuração',0)."' onClick=\"redirect('".$_SERVER['PHP_SELF']."?action=alter&cellStyle=true');\">".
			"</TD><br><BR>";
...
/*			print "<tr><td>COR DO TD_COLOR:</td><td>".
		...
		print "<tr><td colspan='2'>&nbsp;</td></tr>";
		print "<tr><td><input type='submit'  class='button_ok' name='submit' value='".TRANS('BT_ALTER','Alterar')."'></td>";
		print "<td><input type='reset' name='reset'  class='button_ca' value='".TRANS('BT_CANCEL','Cancelar')."' onclick=\"javascript:history.back()\"></td></tr>";
...
<!--
	function valida(){
		...
		var ok = validaForm('idTopo','QUALQUER','Barra principal',1);
		if (ok) var ok =  validaForm('idBarra','COR','Barra de menus',1);
		if (ok) var ok =  validaForm('idMenu','QUALQUER','Fundo do menu lateral',1);
