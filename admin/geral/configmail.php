<?php 
  ...
  */session_start();
	...
	if ((empty($_GET['action'])) and empty($_POST['submit'])){
        		...
        		"<input type='button' class='button_novo' id='idBtIncluir' value='".TRANS('BT_EDIT_CONFIG')."' onClick=\"redirect('".$_SERVER['PHP_SELF']."?action=alter&cellStyle=true');\">".
        	"</TD><br><BR>";
	...
	if ( ((isset($_GET['action']) &&$_GET['action']=="alter")) && empty($_POST['submit'])){
		...
		print "<tr><td><input type='submit'  class='button_ok' name='submit' value='".TRANS('BT_ALTER')."'></td>";
		print "<td><input type='reset' name='reset'  class='button_ca' value='".TRANS('BT_CANCEL')."' onclick=\"javascript:history.back()\"></td></tr>";
	...
	if ($_POST['submit'] = TRANS('BT_ALTER')){
		...
		$exec= mysql_query($qry) or die(TRANS('ERR_EDIT')."<br>".$qry."<br>".  mysql_error());
